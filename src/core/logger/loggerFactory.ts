...
export class LoggerFactory {
   ...
   */
  public static initialize(rootLoglevel: LogLevel = LogLevel.None): void {
    if (!LoggerFactory.isInitialized) {
      // Initialize Root Logger.
      LoggerFactory._loggers.set(
        LoggerFactory.ROOT_LOGGER_NAME,
        new BaseLogger(LoggerFactory.ROOT_LOGGER_NAME, rootLoglevel, [
          new ConsoleAppender(),
        ])
      );
    }
  ...
  public static addLogger(logger: ILogger): void {
    ...
    if (!LoggerFactory.has(logger.name)) {
      LoggerFactory._loggers.set(
        logger.name,
        new BaseLogger(logger.name, logger.level, logger.appenders)
      );
    }
  }
  ...
  public static getLogger(name?: string): Logger {
    LoggerFactory.initialize();
    if (!name) {
      return LoggerFactory._loggers.get(
        LoggerFactory.ROOT_LOGGER_NAME
      ) as Logger;
