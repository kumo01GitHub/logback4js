...
describe("LoggerFactory", () => {
  ...
  it("can't add Logger twice", () => {
    ...
    const appenders2 = [
      new ConsoleAppender(),
      new HttpPostAppender("http://localhost/mock/log"),
    ];
    ...
    expect(LoggerFactory.getLogger(loggerName)).toBeTruthy();
    expect(LoggerFactory.loggers).toHaveLength(beforeLength + 1);
    expect(LoggerFactory.getLogger(loggerName).appenders).toHaveLength(
      appenders1.length
    );
  });
  ...
  it("can remove Logger", () => {
    ...
    expect(LoggerFactory.getLogger(loggerName).name).toEqual(
  ...
  it("returns Root Logger without name", () => {
    expect(LoggerFactory.getLogger().name).toEqual(
      LoggerFactory.ROOT_LOGGER_NAME
    );
  });

  it("returns Root Logger when not added name", () => {
    expect(LoggerFactory.getLogger("notAddedName").name).toEqual(
      LoggerFactory.ROOT_LOGGER_NAME
    );
  });
